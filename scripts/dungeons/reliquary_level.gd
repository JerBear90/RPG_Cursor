extends Node3D
## Sunken Reliquary dungeon level bootstrap.
...
const REGION_ID := "sunken_reliquary"
...
func _ready() -> void:
	...
	if not QuestManager.active_quests.has("depths_of_reliquary") and "depths_of_reliquary" not in QuestManager.completed_quests:
		QuestManager.start_quest("depths_of_reliquary")
...
func _begin_dungeon() -> void:
	...
	if not ready:
		push_error("ReliquaryLevel: collision never ready")
	_register_markers()
...
func _await_collision_ready() -> bool:
	var builder := get_node_or_null("Environment/ReliquaryBuilder")
	if builder == null:
		return false
...
func _register_markers() -> void:
	...
	var cell_size: float = layout.get("cell_size", ReliquaryGenerator.CELL_SIZE)
	...
	for room in layout.get("rooms", []):
		if room is ReliquaryRoom:
			...
			if room.room_type == _Room.RoomType.ENTRANCE:
				entry_center = center
				ReliquaryState.discover_room(room.room_index)
	...
	_SpawnMarker.create_runtime(root, "dungeon_spawn_sunken_reliquary", _SpawnMarker.MarkerType.DUNGEON_ENTRY_SPAWN, entry_center, 0.0, REGION_ID, 4.0)
	_SpawnMarker.create_runtime(root, "dungeon_entry_sunken_reliquary", _SpawnMarker.MarkerType.DUNGEON_ENTRY_SPAWN, entry_center, 0.0, REGION_ID, 4.0)
	if checkpoint_center != Vector3.ZERO:
		_SpawnMarker.create_runtime(root, "dungeon_checkpoint_sunken_reliquary", _SpawnMarker.MarkerType.DUNGEON_CHECKPOINT_SPAWN, checkpoint_center, 0.0, REGION_ID, 5.0)
	if preboss_center != Vector3.ZERO:
		_SpawnMarker.create_runtime(root, "preboss_sunken_reliquary", _SpawnMarker.MarkerType.BOSS_CHECKPOINT_SPAWN, preboss_center, 180.0, REGION_ID, 5.0)
		_SpawnMarker.create_runtime(root, "boss_checkpoint_sunken_reliquary", _SpawnMarker.MarkerType.BOSS_CHECKPOINT_SPAWN, preboss_center, 180.0, REGION_ID, 5.0)
...
func _get_entry_position() -> Vector3:
	for room in DungeonManager.layout.get("rooms", []):
		if room is ReliquaryRoom and room.room_type == _Room.RoomType.ENTRANCE:
