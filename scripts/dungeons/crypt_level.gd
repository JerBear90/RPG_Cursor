extends Node3D
## Paleheart Crypt dungeon level bootstrap.
...
const REGION_ID := "paleheart_crypt"
...
func _ready() -> void:
	...
	if not QuestManager.active_quests.has("gravewind_rising") and "gravewind_rising" not in QuestManager.completed_quests:
		if QuestManager.completed_quests.has("heart_of_blackvein") or InventoryManager.has_item("paleheart_access_key"):
			QuestManager.start_quest("gravewind_rising")
	call_deferred("_begin_dungeon")


func _begin_dungeon() -> void:
	AudioManager.play_music("crypt_ambience")
	var ready := await _await_collision_ready()
	if not ready:
		push_error("CryptLevel: collision never ready")
	_register_markers()
...
func _await_collision_ready() -> bool:
	var builder := get_node_or_null("Environment/CryptBuilder")
	if builder == null:
		return false
...
func _register_markers() -> void:
	...
	var cell_size: float = layout.get("cell_size", _CryptGenerator.CELL_SIZE)
	...
	for room in layout.get("rooms", []):
		if room is CryptRoom:
			...
			if room.room_type == _Room.RoomType.ENTRANCE:
				entry_center = center
				CryptState.discover_room(room.room_index)
	...
	_SpawnMarker.create_runtime(root, "dungeon_spawn_paleheart_crypt", _SpawnMarker.MarkerType.DUNGEON_ENTRY_SPAWN, entry_center, 0.0, REGION_ID, 4.0)
	_SpawnMarker.create_runtime(root, "dungeon_entry_paleheart_crypt", _SpawnMarker.MarkerType.DUNGEON_ENTRY_SPAWN, entry_center, 0.0, REGION_ID, 4.0)
	if checkpoint_center != Vector3.ZERO:
		_SpawnMarker.create_runtime(root, "dungeon_checkpoint_paleheart_crypt", _SpawnMarker.MarkerType.DUNGEON_CHECKPOINT_SPAWN, checkpoint_center, 0.0, REGION_ID, 5.0)
	if preboss_center != Vector3.ZERO:
		_SpawnMarker.create_runtime(root, "preboss_paleheart_crypt", _SpawnMarker.MarkerType.BOSS_CHECKPOINT_SPAWN, preboss_center, 180.0, REGION_ID, 5.0)
		_SpawnMarker.create_runtime(root, "boss_checkpoint_paleheart_crypt", _SpawnMarker.MarkerType.BOSS_CHECKPOINT_SPAWN, preboss_center, 180.0, REGION_ID, 5.0)
...
func _get_entry_position() -> Vector3:
	for room in DungeonManager.layout.get("rooms", []):
		if room is CryptRoom and room.room_type == _Room.RoomType.ENTRANCE:
