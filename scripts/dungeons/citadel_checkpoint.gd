class_name CitadelCheckpoint
extends InteractableBase
## Drowned Citadel storm shelter rest point.
...
func _ready() -> void:
	super._ready()
	prompt_text = "Rest at storm shelter"
...
func _on_interact(_player: Node) -> void:
	if GameManager.in_combat:
		return
	_pending = true
	DialogueManager.start_dialogue("citadel_rest", [
		{"speaker": "Storm Shelter", "text": "Shelter from the coastal storm and bind your path?"},
...
func _on_choice(index: int) -> void:
	...
	WorldStateManager.register_checkpoint("dungeon_checkpoint_drowned_citadel", "drowned_citadel", pos)
	WorldStateManager.dungeon_checkpoint_room = get_meta("room_index", 0)
	CitadelState.checkpoint_activated = true
	CitadelState.save_state()
	if QuestManager.active_quests.has("the_sunken_crown"):
		QuestManager.advance_objective("the_sunken_crown", "reach_checkpoint", 1)
	for hud in get_tree().get_nodes_in_group("game_hud"):
		if hud.has_method("show_toast"):
			hud.show_toast("Checkpoint bound at the storm shelter.")
