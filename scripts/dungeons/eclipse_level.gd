extends Node3D
## Eclipse Sanctum dungeon level bootstrap.
...
const REGION_ID := "eclipse_sanctum"
...
func _begin_dungeon() -> void:
	AudioManager.play_music("sanctum_ambience")
	var ready := await _await_collision_ready()
	if not ready:
		push_error("EclipseLevel: collision never ready")
	_register_markers()
...
func _await_collision_ready() -> bool:
	var builder := get_node_or_null("Environment/EclipseBuilder")
	if builder == null:
		return false
...
func _register_markers() -> void:
	...
	var cell_size: float = layout.get("cell_size", _EclipseGenerator.CELL_SIZE)
	...
	for room in layout.get("rooms", []):
		if room is EclipseSanctumRoom:
			...
			if room.room_type == _Room.RoomType.ENTRANCE:
				entry_center = center
				EclipseSanctumState.discover_room(room.room_index)
			elif room.room_type == _Room.RoomType.CHECKPOINT:
				checkpoint_center = center
			elif room.room_type == _Room.RoomType.ECLIPSE_ANTECHAMBER:
				preboss_center = center
	_SpawnMarker.create_runtime(root, "dungeon_spawn_eclipse_sanctum", _SpawnMarker.MarkerType.DUNGEON_ENTRY_SPAWN, entry_center, 0.0, REGION_ID, 4.0)
	_SpawnMarker.create_runtime(root, "dungeon_entry_eclipse_sanctum", _SpawnMarker.MarkerType.DUNGEON_ENTRY_SPAWN, entry_center, 0.0, REGION_ID, 4.0)
	if checkpoint_center != Vector3.ZERO:
		_SpawnMarker.create_runtime(root, "dungeon_checkpoint_eclipse_sanctum", _SpawnMarker.MarkerType.DUNGEON_CHECKPOINT_SPAWN, checkpoint_center, 0.0, REGION_ID, 5.0)
	if preboss_center != Vector3.ZERO:
		_SpawnMarker.create_runtime(root, "preboss_eclipse_sanctum", _SpawnMarker.MarkerType.BOSS_CHECKPOINT_SPAWN, preboss_center, 180.0, REGION_ID, 5.0)
...
func _get_entry_position() -> Vector3:
	for room in DungeonManager.layout.get("rooms", []):
		if room is EclipseSanctumRoom and room.room_type == _Room.RoomType.ENTRANCE:
