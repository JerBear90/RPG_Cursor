class_name PyreheartCheckpoint
extends InteractableBase
## Pyreheart Ziggurat oasis rest point.
...
func _on_interact(_player: Node) -> void:
	if GameManager.in_combat:
		return
	_pending = true
	DialogueManager.start_dialogue("pyreheart_rest", [
		{"speaker": "Oasis Checkpoint", "text": "Shade and cool stone offer brief relief from the scorching ziggurat.\n\nBind your path here?"},
...
func _on_choice(index: int) -> void:
	...
	WorldStateManager.register_checkpoint("dungeon_checkpoint_pyreheart_ziggurat", "pyreheart_ziggurat", pos)
	WorldStateManager.dungeon_checkpoint_room = get_meta("room_index", 0)
	PyreheartState.checkpoint_activated = true
	PyreheartState.save_state()
	if QuestManager.active_quests.has("heart_of_the_wastes"):
		QuestManager.advance_objective("heart_of_the_wastes", "reach_checkpoint", 1)
	for hud in get_tree().get_nodes_in_group("game_hud"):
		if hud.has_method("show_toast"):
			hud.show_toast("Checkpoint bound at the oasis.")
