class_name FoundryCheckpoint
extends InteractableBase
## Foundry workshop rest point.
...
func _on_interact(_player: Node) -> void:
	if GameManager.in_combat:
		return
	_pending = true
	DialogueManager.start_dialogue("foundry_rest", [
		{"speaker": "Workshop", "text": "Rest at the disabled furnace bench and bind your path?"},
...
func _on_choice(index: int) -> void:
	...
	WorldStateManager.register_checkpoint("dungeon_checkpoint_blackvein_foundry", "blackvein_foundry", pos)
	WorldStateManager.dungeon_checkpoint_room = get_meta("room_index", 0)
	FoundryState.checkpoint_activated = true
	FoundryState.save_state()
	if QuestManager.active_quests.has("heart_of_blackvein"):
		QuestManager.advance_objective("heart_of_blackvein", "reach_checkpoint", 1)
	for hud in get_tree().get_nodes_in_group("game_hud"):
		if hud.has_method("show_toast"):
			hud.show_toast("Checkpoint bound at the workshop.")
