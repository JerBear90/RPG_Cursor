class_name CryptCheckpoint
extends InteractableBase
## Paleheart Crypt sanctum rest point.
...
func _on_interact(_player: Node) -> void:
	if GameManager.in_combat:
		return
	_pending = true
	DialogueManager.start_dialogue("crypt_rest", [
		{"speaker": "Sanctum", "text": "Rest beside the frost-lit brazier and bind your path?"},
...
func _on_choice(index: int) -> void:
	...
	WorldStateManager.register_checkpoint("dungeon_checkpoint_paleheart_crypt", "paleheart_crypt", pos)
	WorldStateManager.dungeon_checkpoint_room = get_meta("room_index", 0)
	CryptState.checkpoint_activated = true
	CryptState.save_state()
	if QuestManager.active_quests.has("the_pale_heart"):
		QuestManager.advance_objective("the_pale_heart", "reach_checkpoint", 1)
	for hud in get_tree().get_nodes_in_group("game_hud"):
		if hud.has_method("show_toast"):
			hud.show_toast("Checkpoint bound at the sanctum.")
