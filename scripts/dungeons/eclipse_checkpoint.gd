class_name EclipseCheckpoint
extends InteractableBase
## Eclipse Sanctum ward rest point.
...
func _on_interact(_player: Node) -> void:
	if GameManager.in_combat:
		return
	_pending = true
	DialogueManager.start_dialogue("eclipse_rest", [
		{"speaker": "Ward Checkpoint", "text": "Warded candles and cold stone offer brief relief from the sanctum's dread.\n\nBind your path here?"},
...
func _on_choice(index: int) -> void:
	...
	WorldStateManager.register_checkpoint("dungeon_checkpoint_eclipse_sanctum", "eclipse_sanctum", pos)
	WorldStateManager.dungeon_checkpoint_room = get_meta("room_index", 0)
	EclipseSanctumState.checkpoint_activated = true
	EclipseSanctumState.save_state()
	if QuestManager.active_quests.has("throne_beneath_the_eclipse"):
		QuestManager.advance_objective("throne_beneath_the_eclipse", "reach_checkpoint", 1)
	for hud in get_tree().get_nodes_in_group("game_hud"):
		if hud.has_method("show_toast"):
			hud.show_toast("Checkpoint bound at the ward.")
